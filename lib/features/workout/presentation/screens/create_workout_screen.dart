...
// Step counter
final createWorkoutCounterProvider = StateProvider<int>((ref) => 0);
final createWorkoutPreviousCounterProvider = StateProvider<int>((ref) => 0);
...
class CreateWorkoutScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final counter = ref.watch(createWorkoutCounterProvider);
    final previousCounter = ref.watch(createWorkoutPreviousCounterProvider);
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      AnimationQuestions(
                        ...
                        counter: counter,
                        child: Text(createWorkoutTitles[counter],
                            style: kTitleCard),
                      ),
                      SizedBox(
                        height: 10,
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(counter < 3 ? 'Next' : 'Save Workout',
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 16,
  ...
  Widget _buildTopBar(BuildContext context, WidgetRef ref) {
    final counter = ref.read(createWorkoutCounterProvider.notifier);
    ref.read(createWorkoutPreviousCounterProvider.notifier);
        ...
        children: [
          GestureDetector(
            ...
            child: Icon(Icons.arrow_back_ios_new,
                color: ref.watch(createWorkoutCounterProvider) == 0
                    ? kLightGrey
                    : kBlack,
                size: 24),
          ),
          Text('Create Workout', style: kPageTitle.copyWith(color: kBlack)),
        ],
      ),
    );
  }
  ...
  void _handleNext(BuildContext context, WidgetRef ref) {
    ...
    if (counter.state == 4) {
      ...
      final currentList = ref.read(allWorkoutsProvider);
      ref.read(allWorkoutsProvider.notifier).state = [
        ...currentList,
        WorkoutModel(
          id: '${Random().nextInt(100)}',
          name: ref.watch(workoutNameProvider),
          description: ref.watch(workoutDescriptionProvider),
          exercises: ref.watch(selectedExercisesProvider),
          type: ref.watch(workoutTypeProvider),
          duration: ref.watch(workoutDurationProvider),
          targetMuscle: ref.watch(selectedMuscleGroupsProvider),
        ),
      ];

      final encoder = JsonEncoder.withIndent('  ');

      for (var i = 0; i < ref.watch(createdWorkoutProvider).length; i++) {
        final prettyJson =
            encoder.convert(ref.watch(createdWorkoutProvider)[i].toJson());
...
class StepContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (counter) {
      case 0:
        return WorkoutInformationView();
      case 1:
        return TargetMusclesView();
      case 2:
        return AddExerciseView();
      case 3:
        return ReviewSaveView();
      default:
        return Center(
